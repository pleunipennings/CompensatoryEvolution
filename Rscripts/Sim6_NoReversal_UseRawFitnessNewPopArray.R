...
muvalues<-1/N*c(0.001)
for (mu in muvalues){
#for (mu in 1/N*c(0.1)){
    Sample10Array<-data.frame(X=c("WT", "Res","CR", paste0("comp",1:10)))
  for (j in 1:20){#starting the sim and  plotting it! 
    ...
    rawfitnessarray=c(0, 1-c, rep(1-(c*(1-comp)),n), 1) #Give WT And CR 0 fitness so only comp works. 
    for (i in 1:G){
    ...
    #write.csv(x = popArrayDF, file = paste0("SimData/","NoReversal_popArrayDF",G,"Gen",comp,"comp",mu,"mu", j, "j.csv"), row.names = FALSE)
    write.csv(x = popArrayDF, file = paste0("SimData/","NoWT_YesCR_popArrayDF",G,"Gen",comp,"comp",mu,"mu", j, "j.csv"), row.names = FALSE)
  ...
  #Don't write these summary files
