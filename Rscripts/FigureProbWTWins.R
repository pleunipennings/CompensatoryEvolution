...
jpeg(filename  = paste0("Output/ProbWTWins", Sys.Date(), ".jpeg"), units = "in", res = 300, width = 8, height = 10)
par(mfrow=c(2,1))

plot(ProbWTWinDet$theta, ProbWTWinDet$probWTWin, log = "x", ylim = c(0,1), 
     type = "l", xlim=c(10^-4, 10), col = 0, 
     main =paste0("Prob. of successful WT reversal mutation before comp. mutations fix (n=100)  "), 
     xaxt="n", yaxt = "n", xlab = "N * mu", ylab = "Prob. WT reversal mutation")
...
legend(x = 0.2, 0.5, legend = paste("p =", c(0.1, 0.2, 0.5, 0.8)), col = 1:4, pch = 2:5, title = "effect of comp. mutations (p)")

plot(ProbWTWinDet$theta, ProbWTWinDet$probWTWin, log = "x", ylim = c(0,1), 
     type = "l", xlim=c(10^-4, 10), col = 0, 
     main =paste0("Prob. of successful WT reversal mutation before comp. mutations fix (p=0.5)  "), 
     xaxt="n", yaxt = "n", xlab = "N * mu", ylab = "Prob. WT reversal mutation")
axis(side = 1, at = c(10^-3, 10^-2, 10^-1, 1, 10), labels = c(10^-3, 10^-2, 10^-1, 1, 10), las = 1)
axis(side = 2, at = seq(0,1,0.1), labels = seq(0,1,0.1), las = 1)
...
legend(x = 0.15, 0.5, legend = paste("n =", c(25,50,100,200)), col = 1:4, pch = 2:5, title = "number of comp. mutations (n)")
